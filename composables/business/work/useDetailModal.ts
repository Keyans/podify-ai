...
import { getWorkFlowTaskDetail } from '~/apis/work/work-flow'
...
export function useDetailModal(): UseCollectorDetailModalReturn { // 🚀 不再接收 tableModalRef
  ...
  const getSubListForTable = async (params: Record<string, any>) => {
    try {
      const res = await getWorkFlowTaskDetail(params);
      if (res.code === 200) {
  ...
  const openCollectorDetailModal = async (record: any) => {
    ...
    currentCollectorId.value = record.workflowId;

    subTitle.value = `工作流: 任务ID | ${record.workflowId}`;
    subStatsData.value = [
