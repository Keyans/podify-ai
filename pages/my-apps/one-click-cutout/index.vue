<template>
    ...
    <div class="bg-white dark:bg-dark-surface rounded-xl shadow-elegant dark:shadow-elegant-dark border border-gray-100 dark:border-dark-border overflow-hidden">
      ...
      <div class="overflow-x-auto">
        <el-table
          ...
          <el-table-column label="抠图类型" width="150">
            <template #default="scope">
              <div class="flex items-center space-x-2">
                <component :is="getTypeIcon(scope.row.type)" :class="getTypeIconColor(scope.row.type)" class="w-5 h-5" />
                <span class="text-sm text-gray-900 dark:text-dark-text">{{ scope.row.type }}</span>
              </div>
            </template>
          </el-table-column>

          <el-table-column prop="imageCount" label="图片数量" width="120">
            <template #default="scope">
  ...
  <!-- 新建抠图对话框 -->
  <ClientOnly>
    <CreateCutoutDialog v-model="showCreateDialog" @success="handleCreateSuccess" />
  </ClientOnly>
...
// 只在客户端导入组件
const CreateCutoutDialog = process.client ? 
  (await import('./components/CreateCutoutDialog.vue')).default : 
  null;
...
// 统计数据
const totalTasks = computed(() => cutoutList.value.length);
const successRate = computed(() => {
  if (cutoutList.value.length === 0) return 0;
  const totalSuccess = cutoutList.value.reduce((sum, item) => sum + item.successCount, 0);
  const totalImages = cutoutList.value.reduce((sum, item) => sum + item.imageCount, 0);
  return totalImages > 0 ? Math.round((totalSuccess / totalImages) * 100) : 0;
});
const processingCount = computed(() =>
  cutoutList.value.filter(item => item.status === '处理中').length
);
const todayCount = computed(() => {
  const today = new Date().toDateString();
  return cutoutList.value.filter(item =>
...
const getTypeIconBg = (type: string) => {
  const bgMap: Record<string, string> = {
    '人像抠图': 'bg-pink-100 dark:bg-pink-900/30',
    '商品抠图': 'bg-blue-100 dark:bg-blue-900/30',
    '物体抠图': 'bg-green-100 dark:bg-green-900/30',
...
const getTypeIconColor = (type: string) => {
  const colorMap: Record<string, string> = {
    '人像抠图': 'text-pink-600 dark:text-pink-400',
    '商品抠图': 'text-blue-600 dark:text-blue-400',
    '物体抠图': 'text-green-600 dark:text-green-400',
...
const batchDownload = () => {
  ...
  ElMessage.success(`正在下载 ${selectedRows.value.length} 个任务的抠图结果...`);
};
...
const loadCutoutList = async () => {
  try {
    ...
    cutoutList.value = mockData.slice(start, end);
